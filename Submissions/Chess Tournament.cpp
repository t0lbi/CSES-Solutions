// Submission Timestamp: 2023-04-25 03:43:08
...
int32_t main(){
	...
	while (t-(tno++)){
		...
		while (st.size()){
			...
			for (int i = 0; i < yapl.size(); ++i)
			{
				ansarr.push_back({yapl[i].second,ele});
				st.erase(yapl[i]);
				yapl[i].first--;
				if (yapl[i].first) st.insert(yapl[i]);
			}
			st.erase({yapl.size(),ele});
		}
		if (!boolean){
