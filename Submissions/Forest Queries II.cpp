// Submission Timestamp: 2022-10-04 09:07:23
...
struct SegTree{
	...
	void toggle(int xnode, int ynode){
		xnode+=segtree.size()/2;
		ynode+=segtree[0].size()/2;
		segtree[xnode][ynode]^=1;
		int crnode = ynode;
		while (crnode>0){
			crnode=(crnode-1)/2;
	...
	int querya(int tarl, int tarr, int flag, int l=0, int r = -1, int node = 0){
		if (r==-1) r = segtree.size() / 2;
		if (l>=tarl && r<=tarr) return segtree[flag][node];
		if (l>tarr || r<tarl) return 0ll;
		int mid = l+(r-l)/2;
		int lnode = querya(tarl, tarr, flag, l, mid, node*2+1);
		int rnode = querya(tarl, tarr, flag, mid+1, r, node*2+2);
		return lnode+rnode;
	}
	int query(int xl, int xr, int yl, int yr, int l = 0, int r = -1, int node = 0){
		if (r==-1) r=segtree.size()/2;
...
int32_t main(){
	...
	while (t-(tno++)){
		...
		while (q--){
			...
			if (type-2){
				deci(x);deci(y);
				segtree.toggle(x-1,y-1);
			}
			else {
				deci(x1);deci(y1);deci(x2);deci(y2);
				x1--,y1--,x2--,y2--;
				cout<<segtree.query(x1,x2,y1,y2)<<endl;
			}
		}
	}
}