// Submission Timestamp: 2022-10-18 10:12:05
...
vector<int> uza;//ok cycle index
vector<int> don;//ok component size
	...
	while (t-(tno++)){
		...
		while (m--){
			...
			if (!dong[a] && !dong[b]){
				...
				int hueh = dist[a].second-dist[b].second;
				if (kthpar(a,hueh)!=b) cout<<-1<<endl;
				else cout<<hueh<<endl;
				continue;
			}
			int rval = 0;
			...
			int bind = don[b];
			if (don[b]<don[a]) bind+=uza[b];
			cout<<rval+bind-don[a]<<endl;
		}
	}
}